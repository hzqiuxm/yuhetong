...
def add_contract():
    """ Add a contract
    :param contract_name, contract_content, appendix,  and part_num (default 2)
    ...
    c_part_num = request.values.get('part_num', 2, type = int) # Default 2 participants
    ...
    with engine.with_session() as ss:
        ...
        c_path = file_biz.save_contract_file(current_user.id, c_content, c_name, c_fuuid)
        draft_uuid = file_biz.generate_file_uuid()
        draft_path = file_biz.save_contract_file(current_user.id, c_content, c_name, draft_uuid)
...
def update_contract(cid):
    ...
    with engine.with_session() as ss:
        ...
        if c_content:
            ...
            file_biz.save_contract_file(contract_to_update.owner_id, c_content,
                                   c_name, draft_to_update.fuuid)
            if is_new_version == 'True' and now_version < 5:
                c_fuuid = file_biz.generate_file_uuid()       # Update contract file -- New file version  4
                c_path = file_biz.save_contract_file(contract_to_update.owner_id,
...
def save_draft(cid):
    ...
    with engine.with_session() as ss:
        contract_to_update = ss.query(LxContract).get(cid)
        draft_to_update = contract_to_update.draft
        file_biz.save_contract_file(contract_to_update.owner_id, d_content,
...
def part_read_contract(cid):
    ...
    with engine.with_session() as ss:
        ...
        now_contract_file = cur_contract.__getattribute__('contract_v' + str(now_version))
        # print now_contract_file.fpath
        contract_content = file_biz.get_contract_content(now_contract_file.fpath)
...
def part_take_contract(cid):
    ...
    with engine.with_session() as ss:
        ...
        ss.add(new_part)
        participation = ss.query(LxContractParticipation).filter_by(
            contract_id = cid)
        if len(participation) == cur_contract.part_num:
            # If all participation take the contract the stage change to
            cur_contract.update({'stage': constants.CONTRACT_STAGE['PARTICIPANTS_TAKE']})
...
def auth_contract(cid):
    ...
    with engine.with_session() as ss:
        ...
        auth_passwd = [random.choice(string.digits + string.ascii_lowercase + string.ascii_uppercase) for i in range(0,8) ]
    ...
    auth_url = 'http://' + config.SERVER_NAME + \
            '/contract/take_auth/' + str(new_auth.id)
...
def get_contract(cid):
    ...
    with engine.with_session() as ss:
        ...
        if not (is_owner or is_partner or has_auth):
            return jsonify({'success': False, 'errorMsg':
                constants.ERROR_CODE['NO_AUTH_CUR_CONTRACT'] })
        ...
        for auth in my_authorizations:
            ...
            if auth.gmt_expire > now:
                my_auth_contracts.append(
                    (auth.contract.id, auth.contract.name, auth.contract.part_num)
                )
        re_dict = dict()
    ...
    with engine.with_session() as ss:
        ...
        if is_part:
            ...
            if not present_part:
                return jsonify({'success': False, 'errorMsg': constants.ERROR_CODE[
                'CONTRACT_AUTH_ERROR']})
            present_part.update({'stage': constants.CONTRACT_PART_STAGE['SIGN']})
        cur_contract = ss.query(LxContract).get(cid)
